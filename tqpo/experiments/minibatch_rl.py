...
class MinibatchRlConstrained(MinibatchRlBase):
    ...
    def train(self):
        ...
        for itr in range(n_itr):
            ...
            with logger.prefix(f"itr #{itr} "):
                ...
                reward, cost = self.algo.transform_sample(samples.env.reward), self.algo.transform_sample(samples.env.env_info.cost)
                reward1 = reward.clone().transpose(0,1).reshape(12,1000).sum(dim=1, keepdim=True)
                cost1 = cost.clone().transpose(0,1).reshape(12,1000).sum(dim=1, keepdim=True)
                self.log_eval(reward1, cost1)

                self.store_diagnostics(itr, traj_infos, opt_info)
                self.log_traj_infos(traj_infos)
                if (itr + 1) % self.log_interval_itrs == 0:
                    self.log_diagnostics(itr)
                    self.plot_traj_info(itr)
        self.shutdown()

    def save_codes(self):
        source_file = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'tqpo.py')
        destination_file = os.path.join(logger.get_snapshot_dir(), 'tqpo.py')
        shutil.copy(source_file, destination_file)

    def plot_traj_info(self, itr):
        traj_summary_filename = os.path.join(logger.get_snapshot_dir(), 'progress.csv')
        df = pd.read_csv(traj_summary_filename)

        plt.figure(1)
        fig, axs = plt.subplots(3,figsize=(10,15))

        axs[0].plot(df['ProbOutage'])
        axs[0].set_title('ProbOutage')
        axs[0].grid(True)
        axs[0].set_ylim([0, 1])
        axs[0].set_yticks([i/10 for i in range(0, 11)])
        axs[0].set_xticks(range(0, len(df['ProbOutage']), 10))

        axs[1].plot(df['ReturnAverage'])
        axs[1].set_title('ReturnAverage')
        axs[1].grid(True)
        axs[1].set_ylim([-5, 30])
        axs[1].set_xticks(range(0, len(df['ReturnAverage']), 10))

        axs[2].plot(df['CostAverage'])
        axs[2].set_title('CostAverage')
        axs[2].grid(True)
        axs[2].set_ylim([0, 50])
        axs[2].set_xticks(range(0, len(df['CostAverage']), 10))

        plt.savefig(os.path.join(logger.get_snapshot_dir(), f'traj_info_plot.png'))
    ...
    def log_eval(self, reward, cost):

        traj_summary_filename = os.path.join(
            logger.get_snapshot_dir(),
            'traj_eval_summary.csv',
        )
        is_first_row = not os.path.exists(traj_summary_filename)
        summary_fd = open(traj_summary_filename, 'a', newline='')
        summary_writer = csv.writer(summary_fd)

        for reward1,cost1 in zip(reward, cost):
            # Compute Summary
            summary_dict = dict()
            summary_dict['TrajSumReward'] = reward1.item()
            summary_dict['TrajSumCost'] = cost1.item()
