...
from rlpyt.projects.tqpo.experiments.exp_launcher import run_experiments_serial

script = osp.abspath(osp.join(osp.dirname(__file__), 'train_tqpo.py'))
print(script)

default_config_key = "LSTM"
experiment_title = f"tqpo"
cost_limit = 15
target_prob = 0.1
n_steps = 5e6
log_interval_steps = 5e4

n_cpu = psutil.cpu_count() // 2

affinity_code = f'{n_cpu}cpu_0gpu_1cpr_1cpw_0hto'
run_slot = 0
n_run = 5 # number of serial run
assert run_slot < n_cpu, "run_slot should be less than n_cpu"

print('rlpyt.projects.tqpo.experiments.launch_tqpo.py | affinity_code : ', affinity_code)
print('rlpyt.projects.tqpo.experiments.launch_tqpo.py | run_slot : ', run_slot)

variant_levels = list()

env_ids = ['SimpleEnv-v0']
